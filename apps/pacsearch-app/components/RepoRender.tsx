    ...
    return (
        <div className="flex flex-col border-solid border-cyan-500 w-auto p-6">
            <table className='table-auto border-collapse border border-blue-400 p-5'>
                <tbody 
                  className='border-collapse border border-blue-400 p-5 text-center items-center justify-items-center'
                >
                    {table.getRowModel().rows.map(row => (
                        ...
                        {row.getVisibleCells().map(cell => (
                            <td 
                                ...
                                className={cell.row.original.name === name ? 'bg-blue-500 text-white' : 'bg-black'}
                            >
